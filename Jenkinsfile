...
podTemplate(
   ...
   containers: [
      containerTemplate(
         ...
         envVars: [envVar(key: 'DOCKER_CONFIG', value: '/home/jenkins/.docker/')],
      ),
      containerTemplate(
         name: 'ibmcloud',
   ...
   serviceAccount: 'jenkins'
    ...
    node(buildLabel) {
        container(name: 'node', shell: '/bin/bash') {
            git credentialsId: 'git-credentials', url: 'https://github.ibm.com/garage-catalyst/accelerate-api.git'
            stage('Setup') {
                sh '''
